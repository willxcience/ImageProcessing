...
if (isset($_POST['submit'])){
    ...
    foreach ($_FILES['files']['name'] as $i => $name ) {
        ...
        if ($_FILES['files']['error'][$i] == 0) {
			...
			$fileTime = date('r', filectime($folderpath));
			echo $fileTime;
            $sql = "INSERT INTO image (foldername, size, type, path, folderpath, location, date) ".
            "VALUES ('$foldername', '$fileSize', '$fileType', '$target_file', '$folderpath' ,'$location' ,'$fileTime')";
            
            $mysqli->query($sql) or die("Error, updating query failed" . mysqli_error($mysqli));

            if (move_uploaded_file($_FILES['files']['tmp_name'][$i], $target_file)) {
            	$progre=$progre+1;
            	echo "progress : ".$progre."  ";
                echo "The SP file ". basename( $_FILES['files']['name'][$i]). " has been uploaded.<br></br>";
            }
